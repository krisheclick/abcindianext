    ...
    <section className="project-listing">
      <div className="container">
        ...
        {projectCategories?.length > 0 && (
          <ul className={`${Styles.gllist} d-flex align-items-center justify-content-center`}>
            {projectCategories.map((cat) => (
              <li key={cat.project_category_slug}>
                <a href="">{cat.project_category_title}</a>
              </li>
            ))}
          </ul>
        )}
        ...
        <div className="project-list">
          {projects.map((project) => {
            ...
            return ( 
              <div className={`${Styles.inner_mdlbxsin} d-flex align-items-center`} key={project.project_id}>

                <div className={Styles.inrplsbl}>
                  {/* CATEGORY */}
                  {project.Category && (
                  ...
                  <div className={Styles.inrplsblaltxt}>
                    <div className={Styles.inrplsblaltxth}>{project.project_title}</div> 
  
                    {/* DESCRIPTION */}
                    <div className={Styles.inrplsblaltxtp}
                    ...
                    <div className={Styles.inrplsbldbx}>
                     {location?.text && location?.url && (
                        <a href={location.url} className={`${Styles.inrplsbldbxs} d-flex align-items-center`}>
                          <div className={Styles.inrplsbldbxstick}><i className="fa-solid fa-location-dot"></i></div>
                    ...
                    <div className={`${Styles.lmwv} d-flex align-items-center`}>
                      {button?.text && button?.url && (
                        <a href={button.url} className={Styles.lmwvlm}>
                          {button.text}
                        </a>
                      )}  
                      {button?.text && button?.url && (
                        <a href={button.url} className={Styles.lmwvwov}>
                          {button.text}
                        </a>
                      )}  
                    </div>
                ...
                <div className={Styles.inrplsbr}>
                  <figure className={Styles.plimg}>  
                  {/* IMAGE */}
